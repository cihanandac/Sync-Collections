...
from .models import MuseumObject, ObjectLog, SyncLock

collection_types_variable = config("COLLECTION_TYPES", default="VanabbeCollectie")
...
def not_synced_objects(request):
    ...
    return render(
        request,
        "collection/index.html",
        {"museum_objects": page_obj, "sync_running": sync_running},
    )
...
def check_status(request, ccObjectID):
    ...
    return render(
        request,
        "collection/index.html",
        {"museum_objects": object, "sync_running": sync_running},
    )
...
def syncStartAdlib(request):
    ...
    return JsonResponse(
        {"message": "Only POST and GET requests are allowed!"}, status=405
    )
        ...
        if sync_running == False:
            ...
            if data.get("action") == "run":
                if not sync_running:
                    ...
                    sync.save()
                    return JsonResponse(
                        {"message": f"Synced object {ccObjectID} successfully!"}
                    )
                else:
                    return JsonResponse(
                        {"message": "Sync is already running!"}, status=409
                    )
            else:
                return JsonResponse({"message": "Invalid action!"}, status=400)
    return JsonResponse({"message": "Only POST requests are allowed!"}, status=405)
    ...
    try:
        for collection in collection_types:
            if sync.stop_requested:
                ...
                sync_start_logger.info(f"User requested stop at {stopped_time}")
                break
        ...
        create_update_object_logger.info(f"Sync process finished at {end_time}")
...
def sync_database(collection):
    ...
    for offset in range(0, int(count), 10):
        ...
        for record in records:
            ...
            if timestamp:
                timestamp_parsed = datetime.strptime(timestamp, "%d-%m-%Y %H:%M")
...
def create_update_object(museum_object):
    ...
    try:
        response = requests.get(
            plone_url, auth=HTTPBasicAuth(plone_username, plone_password), timeout=1800
        )
    ...
    ObjectLog.objects.create(museum_object=museum_object, log_message=log_message)
...
def show_museum_object_logs(request, object_id):
    ...
    return render(
        request,
        "collection/show_logs.html",
        {"museum_object": museum_object, "logs": logs},
    )
